...
# ══════════════════════════════════════════════════════════════════════════
st.set_page_config(page_title="Stock Hunter Pro v10.0", layout="wide")

if "refresh_key" not in st.session_state:
    st.session_state.refresh_key = 0

st.sidebar.markdown("## 🦅 Stock Hunter Pro v10.0")
st.sidebar.markdown("### `The Pure Risk Engine`")
st.sidebar.caption("🔒 โฟกัสเฉพาะคณิตศาสตร์การคุมเงินทุน — ตัดระบบข่าวและสัญญาณหลอกทิ้ง 100%")
st.sidebar.divider()
...
# ──────────────────────────────────────────────────────────────────────────
st.sidebar.error(
    "⚠️ **คำเตือนเดย์เทรด:** ระบบนี้ใช้สูตร EMA Crossover + ATR สำหรับคำนวณหน้างาน "
    "ไม่ใช่การพยากรณ์ราคาอนาคต ในสภาวะตลาดไซด์เวย์ (Sideways) สัญญาณจะกลับไปกลับมาบ่อย "
...
def fetch_timeframe_data(ticker, interval="1h", _state_key=0):
    ...
    try:
        ...
        # กรองราคาที่ผิดปกติ (0, ติดลบ, NaN) ทิ้งก่อนคำนวณต่อ
...
def safe_signal_block(df_proc, current_price):
    """ รวม logic คำนวณสัญญาณ + กัน edge case ไว้จุดเดียว ลดโอกาส bug ซ้ำซ้อน """
    ...
    # กันกรณีค่าเบี่ยงเบนราคาแคบจนเกือบเป็นศูนย์ (Micro-cent variance)
...
def compute_position_size(sig, account_capital, risk_per_trade_pct, max_position_pct):
    """ คำนวณจำนวนหุ้น โดยบังคับทั้งสองเพดานความปลอดภัย """
    try:
        ...
        return {"shares": 0, "cost": 0.0, "max_loss_allowed": 0.0, "capped_by_position_limit": False}

def compute_correlation_matrix(price_data: dict):
    """ คำนวณ correlation ของ % การเปลี่ยนแปลงราคาระหว่างหุ้นในลิสต์ """
    closes = {}
...
if menu == "⚡ 1. คำนวณขนาดไม้เทรด (Position Sizing) & สแกนสด":
    ...
    st.markdown("คำนวณปริมาณหุ้นที่ควรซื้อและจุดจำกัดการขาดทุน อ้างอิงกฎ **Risk per Trade**")
    st.caption("⚠️ สัญญาณ BUY/SELL ด้านล่างมาจากการตัดกันของเส้นค่าเฉลี่ย (EMA crossover) เป็นข้อมูลเชิงสถิติ ไม่ใช่การพยากรณ์ราคาอนาคต")
            ...
            if df_proc is not None:
                ...
                if sig is not None:
                    ...
                    scanned_data.append({
                        ...
                        "เป้ากำไร (TP)": "-", "จุดตัดขาดทุน (SL)": "-", "จำนวนหุ้นสูงสุดที่คำนวณได้": "-", "เงินทุนที่ใช้ในไม้นี้": "-",
                    })
            else:
                scanned_data.append({
    ...
    if scanned_data:
        ...
        if corr_matrix is not None and len(corr_matrix) >= 2:
            ...
            st.caption("ถ้าหุ้นสองตัวมีค่าสหสัมพันธ์สูงกว่า 0.7 แปลว่ามักวิ่งไปทางเดียวกัน การเปิดไม้พร้อมกันเป็นการเพิ่มความเสี่ยงทับซ้อน")
    ...
    st.caption("⚠️ ผลลัพธ์ด้านล่างคำนวณจากสูตรคณิตศาสตร์เท่านั้น โปรดใช้ประกอบการตัดสินใจ")
    ...
    if not t_stock:
        ...
        if df_proc is not None:
            ...
            if sig is None:
                st.error("ข้อมูลของหุ้นนี้ไม่น่าเชื่อถือพอจะคำนวณสัญญาณ กรุณาลองหุ้นอื่นหรือ Timeframe อื่น")
            else:
                pos = compute_position_size(sig, account_capital, risk_per_trade, max_position_pct)
                signal_icon = "🟩" if sig["signal"] == "BUY / LONG" else "🟥"
                ...
                cx2.markdown(f"🤖 **สัญญาณเชิงสถิติ:** \n## {signal_icon} {sig['signal']}")
                cx3.metric("🎯 เป้าหมายกำไร (TP)", f"${sig['tp']:.2f}")
                cx4.metric("🛑 จุดตัดขาดทุน (SL)", f"${sig['sl']:.2f}")

                cap_note = f" *(จำนวนนี้ถูกจำกัดโดยเพดานสัดส่วนต่อหุ้น {max_position_pct}% ของพอร์ต)*" if pos["capped_by_position_limit"] else ""
                st.info(f"🛡️ **ขนาดไม้ตามกฎความเสี่ยง:** ไม่เกิน **{pos['shares']} หุ้น** (ใช้เงิน ~${pos['cost']:,.2f}, ขาดทุนสูงสุดหากโดน SL ≈ ${pos['max_loss_allowed']:,.2f}){cap_note}")

                if df_proc['RSI'].iloc[-2] > 70:
                    st.warning("📌 RSI อยู่ในโซน Overbought (>70) — ระวังการย่อตัว")
                elif df_proc['RSI'].iloc[-2] < 30:
                    st.warning("📌 RSI อยู่ในโซน Oversold (<30) — ระวังการเด้งกลับ")
            ...
            st.error("ไม่สามารถเชื่อมข้อมูลหลักทรัพย์นี้ได้ กรุณาตรวจสอบสัญลักษณ์หุ้น")
